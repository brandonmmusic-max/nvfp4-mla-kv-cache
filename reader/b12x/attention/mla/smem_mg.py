...
def _run_module_asserts() -> None:
    ...
    # NVFP4 (E2M1 + E4M3 group-16 scales, GLM-family): BF16-QK layout with the
    # 288-byte packed KV row (256B E2M1 + 32B scales; the record's 16B pad and
    # 128B rope are NOT staged -- rope is read from global/L2 like GLM). Q-NoPE
    # is staged BF16 at D_NOPE+8 stride; BF16 probabilities for P.V reuse the
    # dead W_FP8 region at sm_p_full_stride like the DSV4 XV-RoPE weights.
    ...
    for nhg in (1, 2):
        ...
        assert nl.kv_sc_buf_bytes == 0  # E4M3 scales travel in the 288B row.
        assert nl.kv_rope_buf_bytes == 0  # rope from global/L2 (GLM MG design).
        assert nl.q_nope_bf16_stride == 512 + 8
        assert nl.q_rope_off == nl.w_fp8_off  # Q-rope scratch aliases W_FP8.
        assert nl.sm_p_full_off == nl.w_fp8_off  # BF16 P tile aliases W_FP8.
