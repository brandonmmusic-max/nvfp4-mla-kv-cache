...
class CompressorStateCache(torch.nn.Module, AttentionLayerBase):
    ...
    def get_kv_cache_spec(self, vllm_config: VllmConfig) -> KVCacheSpec:
        # Custom DS MLA layouts need record-size page alignment. Plain full-cache
        # rows share state pages with contiguous KV pages, so padding would break
        # page matching.
        cache_dtype = vllm_config.cache_config.cache_dtype
        uses_fp8_ds_mla_layout = cache_dtype == _DSV4_FP8_DS_MLA_DTYPE
        uses_nvfp4_ds_mla_layout = cache_dtype == _DSV4_NVFP4_DS_MLA_DTYPE
...
class DeepseekCompressor(nn.Module):
    ...
    def forward(
        ...
        if self.vllm_config.cache_config.cache_dtype == _DSV4_NVFP4_DS_MLA_DTYPE:
            raise RuntimeError(
                "nvfp4_ds_mla is not end-to-end enabled in this v2 port: "
                "DeepseekCompressor writes fp8_ds_mla via "
                "compress_norm_rope_store_triton/compress_norm_rope_store_cutedsl. "
                "A matching NVFP4 compressor writer for 432B records is required."
            )
