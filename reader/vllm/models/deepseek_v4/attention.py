...
_DSV4_FP8_DS_MLA_DTYPE = "fp8_ds_mla"
_DSV4_NVFP4_DS_MLA_DTYPE = "nvfp4_ds_mla"
_DSV4_NVFP4_DS_MLA_ALIGNMENT = 432
...
def _resolve_dsv4_kv_cache_dtype(
    ...
    experimental ``nvfp4_ds_mla`` string keeps a separate 432B packed record.
    Plain-row backends store each token's KV row in its element dtype: bf16 or
    per-tensor FP8 E4M3.
    """
    if use_fp8_ds_mla_layout:
        # Custom DeepseekV4 packed MLA formats are byte-backed. fp8 aliases keep
        # the existing fp8_ds_mla behavior; nvfp4_ds_mla is intentionally kept
        # distinct so the 432B layout can be sized and guarded separately.
        assert kv_cache_dtype == _DSV4_NVFP4_DS_MLA_DTYPE or kv_cache_dtype.startswith(
            "fp8"
        ), (
            "DeepseekV4 packed MLA layout only supports fp8_ds_mla or "
            f"nvfp4_ds_mla kv-cache, got {kv_cache_dtype}"
        )
        if kv_cache_dtype != _DSV4_NVFP4_DS_MLA_DTYPE and (
            kv_cache_dtype != _DSV4_FP8_DS_MLA_DTYPE
        ):
            if cache_config is not None:
                cache_config.cache_dtype = _DSV4_FP8_DS_MLA_DTYPE
            kv_cache_dtype = _DSV4_FP8_DS_MLA_DTYPE
...
class DeepseekV4Attention(nn.Module, AttentionLayerBase, ABC):
    ...
    # = packed DS MLA byte layout (fp8_ds_mla, or guarded nvfp4_ds_mla); False =
    # plain bf16 / per-tensor fp8 KV row. Backends can override the instance hook
    # when a single attention class dispatches across arch-specific layouts.
    ...
    def _uses_fp8_ds_mla_layout(self) -> bool:
        """Return whether this instance stores KV in the packed DS MLA layout."""
    ...
    def _fused_qnorm_rope_kv_insert(
        ...
        if cache_dtype == torch.uint8:
            if self.kv_cache_dtype == _DSV4_NVFP4_DS_MLA_DTYPE:
                raise RuntimeError(
                    "nvfp4_ds_mla is not end-to-end enabled in this v2 port: "
                    "the DeepSeek-V4 SWA fused writer "
                    "fused_deepseek_v4_qnorm_rope_kv_rope_quant_insert writes "
                    "the fp8_ds_mla UE8M0 584B layout. A matching NVFP4 SWA "
                    "writer is required before decode can read 432B records."
                )
    ...
    def get_kv_cache_spec(self, vllm_config: VllmConfig) -> KVCacheSpec | None:
        if (
            ...
            return None
        # Packed DS MLA layouts are uint8 and need record-size alignment; plain
        # bf16 / per-tensor fp8 rows use natural element-size pages.
        uses_fp8_ds_mla_layout = self.kv_cache_dtype == _DSV4_FP8_DS_MLA_DTYPE
        uses_nvfp4_ds_mla_layout = self.kv_cache_dtype == _DSV4_NVFP4_DS_MLA_DTYPE
        return MLAAttentionSpec(
            ...
            alignment=(
                ...
                else _DSV4_NVFP4_DS_MLA_ALIGNMENT
