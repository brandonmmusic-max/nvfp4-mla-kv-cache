...
class DeepseekV4FlashInferMLASparseBackend(DeepseekV4FlashMLABackend):
    ...
    def supports_combination(
        ...
        if device_capability.major == 10:
            if kv_cache_dtype in ("fp8_ds_mla", _DSV4_NVFP4_DS_MLA_DTYPE):
                return (
                    "FLASHINFER_MLA_SPARSE_DSV4 SM10x uses the plain "
                    "per-tensor FP8 KV layout, not packed DS MLA layouts"
                )
...
class DeepseekV4FlashInferSM120Attention(DeepseekV4Attention):
    ...
    def _raise_nvfp4_reader_missing(self) -> None:
        raise RuntimeError(
            "nvfp4_ds_mla is not end-to-end enabled in this v2 port: "
            "FlashInfer decode/prefill calls "
            "flashinfer.decode.trtllm_batch_decode_sparse_mla_dsv4, whose v2 "
            "contract reads fp8_ds_mla 584B UE8M0 pages. Missing required op: "
            "an NVFP4 DSv4 sparse MLA reader/dequant variant accepting 432B "
            "records (256B E2M1 NoPE, 32B E4M3 group-16 scales, 16B pad, "
            "128B bf16 RoPE) for both swa_kv_cache and compressed_kv_cache."
        )
    ...
    def _forward_decode(
        ...
        if self.kv_cache_dtype == _DSV4_NVFP4_DS_MLA_DTYPE:
            self._raise_nvfp4_reader_missing()
        flashinfer_trtllm_batch_decode_sparse_mla_dsv4(
            query=q,
