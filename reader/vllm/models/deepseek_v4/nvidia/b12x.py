...
def _raise_nvfp4_ds_mla_b12x_reader_missing() -> None:
    raise RuntimeError(
        "nvfp4_ds_mla is not end-to-end enabled in this v2 port: "
        "b12x.integration.mla.compressed_mla_decode_forward reads the "
        "fp8_ds_mla DSv4 584B UE8M0 page format and its Python binding has no "
        "kv_cache_dtype/scale_format/record_bytes parameter. Missing required "
        "op/signature: a B12X compressed MLA decode+extend reader that accepts "
        "432B NVFP4 DSv4 records (256B E2M1 NoPE, 32B E4M3 group-16 scales, "
        "16B pad, 128B bf16 RoPE) for both SWA and indexed caches."
    )
...
class DeepseekV4B12xMLAAttention(DeepseekV4FlashMLAAttention):
    ...
    def _forward_decode(
        ...
        if self.kv_cache_dtype == _DSV4_NVFP4_DS_MLA_DTYPE:
            _raise_nvfp4_ds_mla_b12x_reader_missing()
        if not swa_only:
