...
def concat_and_cache_nvfp4_mla(
    ...
    """Store BF16/FP16 MLA latent as the packed 432B NVFP4 record.

    256B E2M1 NoPE (per-16-group E4M3 scales at +256, 16B pad at +288) and the
    RoPE dims copied BIT-EXACT as the source 16-bit values at +304. Implemented
    by /opt/nvfp4_extra/_C_nvfp4.so (loaded above); raises a clear error if the
    extension was not loadable in this environment.
    """
    if not hasattr(torch.ops._C_cache_ops, "concat_and_cache_nvfp4_mla"):
        raise RuntimeError(
            "nvfp4_ds_mla KV cache requires the NVFP4 MLA writer extension "
            "(/opt/nvfp4_extra/_C_nvfp4.so); it was not loaded. "
            "[nvfp4_reader_port]"
        )
