...
def get_kv_quant_mode(kv_cache_dtype: str) -> KVQuantMode:
    ...
    if kv_cache_dtype == "nvfp4_ds_mla":
        # Packed 432B/token DeepSeek-family MLA record (E2M1 NoPE + E4M3
        # group-16 scales + BF16 RoPE). Page bytes come from the MLA spec's
        # cache_dtype_str branches, NOT from the per-head NVFP4 split-view
        # machinery, so deliberately mirror fp8_ds_mla's quant-mode treatment.
...
class SlidingWindowMLASpec(SlidingWindowSpec):
    ...
    def real_page_size_bytes(self) -> int:
        if self.model_version == "deepseek_v4" and self.cache_dtype_str in (
            ...
            # DeepseekV4 FlashMLA: fp8_ds_mla is 584B/token (448B NoPE + 128B
            # RoPE + 8B fp8 scale); nvfp4_ds_mla is the 432B/token E2M1 +
            # E4M3-group-16-scale record. FlashInfer's contiguous bf16/fp8
            # cache falls through to the element-size formula below.
            record_bytes = 432 if self.cache_dtype_str == "nvfp4_ds_mla" else 584
